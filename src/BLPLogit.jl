...
function cshare(delta::Vector, p::Vector, char1::Vector, v1, v2, σ)
    ns = length(v1)
    k = length(p)
    share = zeros(k)
    σ1 = σ[1]
    ...
    @threads for g in 1:ns
        num = exp.(delta - σ1 * v1[g] * p + σ2 * v2[g] * char1)
        den = 1 + sum(num)
        prob = num./den
        share += 1/ns * prob
    end
...
function cdelta(share::Vector,p::Vector, char1::Vector, v1, v2, σ,tol)
    k = length(p)
    delta0 = ones(k)
    delta1 = ones(k)
    error = 1
    ...
    while error > tol
        delta0 = delta1
        j = cshare(delta0, p, char1, v1, v2, σ)
...
function delta_full(σ, df, v1, v2)
    ...
    @threads for j in 1:m
        ...
        δ_new = cdelta(share, p , char1, v1, v2, σ,tol)
...
function obj(σ ,df, v1, v2)
    δ = delta_full(σ, df, v1, v2)
    p = df[:, :p]
    char1 = df[: , :char1]
    char2 = df[: , :char2]
    share = df[: , :share]
